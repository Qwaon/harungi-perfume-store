...
export const metadata = {
  title: 'О нас — HARUNGI',
  description: 'Кто мы такие, как работаем и почему нам можно доверять.',
};
...
const reasons = [
  ...
  },
  {
    ...
    desc: 'Разливаем из запечатанных оригинальных флаконов в чистые атомайзеры. Вы получаете именно тот аромат, что в каталоге.',
  },
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
    ...
    title: 'Без переплат',
    desc: 'Мы — небольшой магазин без аренды и большого штата. Цены честные, без накрутки за «бутик».',
  },
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
    ...
    desc: 'Отвечаем в Telegram быстро. Поможем с выбором, расскажем про стойкость, посоветуем аналоги.',
  },
];
    ...
    <div className="min-h-screen pt-28 md:pt-36">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="mb-20 md:mb-28 max-w-2xl">
          <p className="label text-ink-300 mb-4">О нас</p>
          ...
          <p className="text-ink-500 text-base md:text-lg leading-relaxed">
            Мы — небольшой магазин нишевой и селективной парфюмерии. Продаём оригиналы и делаем распивы из оригинальных флаконов, чтобы вы могли попробовать аромат перед покупкой полной версии.
          </p>
        </div>

        {/* Reasons */}
        <section className="mb-24 md:mb-32">
          <p className="label text-ink-300 mb-3">Почему мы</p>
          <h2 className="section-title mb-12">Наши принципы</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {reasons.map((r) => (
              <div
                key={r.title}
                className="flex gap-5 p-8 bg-white rounded-2xl border border-cream-200 hover:border-cream-300 hover:shadow-sm transition-all duration-200"
              >
                <div className="w-10 h-10 rounded-xl bg-cream-100 flex items-center justify-center shrink-0 text-ink-700">
                  {r.icon}
                </div>
                <div>
                  <h3 className="font-display text-xl font-medium text-ink-900 mb-2">{r.title}</h3>
        ...
        {/* How it works */}
        <section className="mb-24 md:mb-32">
          <p className="label text-ink-300 mb-3">Процесс</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {steps.map((s) => (
              <div key={s.num}>
                ...
                <h3 className="font-display text-xl font-medium text-ink-900 mb-2">{s.title}</h3>
        ...
        <section className="border-t border-cream-200 pt-16 pb-20 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          <div>
            ...
            <p className="text-ink-400 text-sm">Распивы от 2 мл — минимальный риск, максимальное удовольствие.</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-4 shrink-0">
            ...
            <a
              ...
              className="inline-flex items-center justify-center gap-2.5 text-white text-sm font-medium px-8 py-3.5 rounded-lg transition-all duration-200 hover:opacity-90 hover:-translate-y-px hover:shadow-lg tracking-widest uppercase"
