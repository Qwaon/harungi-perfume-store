...
**Architecture:** Подход A. Серверные Route Handlers (`/api/admin/*`) пишут в Supabase под `service_role` (server-only); `middleware.ts` защищает `/admin/*` и `/api/admin/*` подписанной httpOnly-cookie. Service_role-ключ в браузер не попадает. Логика нормализации/валидации каталога копируется из `workers/admin-bot.js` (известный долг — менять обе копии синхронно).
...
### Task 1: Чистая логика каталога (копия из бота) + тесты

Копируем проверенные функции из `workers/admin-bot.js` в `src/lib/admin/`, чтобы веб-формы использовали ту же нормализацию/валидацию/slug. Это TS-копия (бот остаётся на JS).
...
// КОПИЯ чистых функций из workers/admin-bot.js (бот — отдельный Worker на JS).
// ⚠️ Технический долг: при изменении править ОБЕ копии синхронно.
...
- Технический долг (копия логики бота) → отмечено в Task 1 ✓
